/**
 * Step 7: 컨트랙트 제출 (Backend Simulation)
 * UserVoteBatch[] 구조로 데이터 제출
 */
 ...
 */
function generateRecordId(index: number): string {
  // 타임스탬프 + 인덱스를 사용하여 유니크 ID 생성
 ...
 * Step 7: 컨트랙트 제출 컴포넌트
 */
export class Step7Submit extends BaseStep {
  private onEventsReceived: ((receipt: any, txHash: string) => void) | null = null;
   ...
   * Step 9 연동을 위한 콜백 설정
   */
  setEventCallback(callback: (receipt: any, txHash: string) => void): void {
          ...
          <div id="remixParams" class="mt-3 p-4 bg-blue-50 rounded-lg border border-blue-200">
            ...
            <div class="mb-4 bg-white rounded border border-blue-300 p-3">
              <div class="flex items-center justify-between mb-2">
                ...
                <button id="copyBatchesBtn" class="bg-blue-500 text-white px-3 py-1 rounded text-xs hover:bg-blue-600">
            ...
            <div class="mb-4 bg-white rounded border border-blue-300 p-3">
              <div class="flex items-center justify-between mb-2">
                ...
                <button id="copyBatchNonceBtn" class="bg-blue-500 text-white px-3 py-1 rounded text-xs hover:bg-blue-600">
                  <i data-lucide="clipboard" class="w-3 h-3 inline"></i> 복사
                </button>
              </div>
            ...
            <div class="mb-4 bg-white rounded border border-blue-300 p-3">
              <div class="flex items-center justify-between mb-2">
                ...
                <button id="copyExecutorSigBtn" class="bg-blue-500 text-white px-3 py-1 rounded text-xs hover:bg-blue-600">
                  <i data-lucide="clipboard" class="w-3 h-3 inline"></i> 복사
                </button>
              </div>
  ...
  private bindEvents(): void {
    ...
    // 복사 버튼들
    const copyBatchesBtn = document.getElementById('copyBatchesBtn');
    const copyBatchNonceBtn = document.getElementById('copyBatchNonceBtn');
    const copyExecutorSigBtn = document.getElementById('copyExecutorSigBtn');

    if (copyBatchesBtn) {
      copyBatchesBtn.addEventListener('click', () => this.copyParam('batches'));
    }
    if (copyBatchNonceBtn) {
      copyBatchNonceBtn.addEventListener('click', () => this.copyParam('batchNonce'));
    }
    if (copyExecutorSigBtn) {
      copyExecutorSigBtn.addEventListener('click', () => this.copyParam('executorSig'));
    }
  }
  ...
  private updateSummary(): void {
    ...
    // userBatchSigs에서 user address를 가져와서 userId와 매핑
    if (state.records && state.records.length > 0 && state.userBatchSigs && state.userBatchSigs.length > 0) {
      const uniqueUsers = new Map<string, string>();

      // userBatchSigs의 user address와 records의 userId를 매핑
      state.userBatchSigs.forEach((sig) => {
        // 해당 user의 첫 번째 레코드에서 userId 가져오기
        const firstRecord = state.records.find((r) => r.userId !== ''); // 임시로 userId 기준
        if (firstRecord && !uniqueUsers.has(sig.user)) {
          uniqueUsers.set(sig.user, firstRecord.userId);
        }
      });

      const mappingHTML = Array.from(uniqueUsers.entries())
        .map(
          ([address, userId]) =>
            `<div class="text-green-700 flex items-center gap-1">
              <i data-lucide="check-circle" class="w-4 h-4"></i> ${address.slice(0, 6)}...${address.slice(-4)} → userId: "${userId}"
            </div>`
        )
        .join('');

      UIHelper.setInnerHTML('userIdMappingList', mappingHTML);
      renderIcons();
    }

    // 레코드 수
    const recordCount = state.records?.length || 0;
    UIHelper.setText('submitRecordCount', String(recordCount));

    // User Batch 수
    const userBatchCount = state.userBatchSigs?.length || 0;
    UIHelper.setText('submitUserBatchCount', String(userBatchCount));

    // 사용자별 레코드 수 계산 (userBatchSigs 기준)
    if (state.records && state.records.length > 0 && state.userBatchSigs && state.userBatchSigs.length > 0) {
      const breakdown = state.userBatchSigs
        .map((_sig, idx) => {
          // 현재는 모든 레코드가 같은 유저의 것으로 가정 (Step2에서 생성)
          const count = state.records.length / state.userBatchSigs.length;
          return `User ${idx + 1}: ${Math.ceil(count)}개`;
        })
        .join(', ');
      UIHelper.setText('submitUserRecordBreakdown', breakdown);
    }

    // Batch Nonce
    UIHelper.setText('submitBatchNonce', state.batchNonce !== undefined ? String(state.batchNonce) : '-');

    // Executor 서명
    const execSig = state.executorSig ? '있음' : '없음';
    UIHelper.setText('submitExecutorSig', execSig);
  }

  /**
   * UserVoteBatch[] 구조로 데이터 생성 (Remix 표시용)
   */
  private buildUserVoteBatches(): any[] {
    const state = this.state.getState();
    const batches: any[] = [];
    let globalIndex = 0;

    // 각 userBatchSig에 대해 UserVoteBatch 생성
    // 현재 구조: 모든 레코드를 각 유저에게 동일하게 할당 (Step2 로직에 맞춤)
    state.userBatchSigs.forEach((sig) => {
      // 모든 레코드를 해당 유저의 것으로 처리 (데모 목적)
      const userRecords = state.records.map((r) => {
  ...
  private generateRemixParams(): void {
    const state = this.state.getState();
    ...
    // 각 파라미터를 개별적으로 설정
    const batchesElement = document.getElementById('remixParam_batches') as HTMLTextAreaElement | null;
    const batchNonceElement = document.getElementById('remixParam_batchNonce') as HTMLInputElement | null;
    const executorSigElement = document.getElementById('remixParam_executorSig') as HTMLInputElement | null;
    ...
    if (batchNonceElement) {
      batchNonceElement.value = state.batchNonce !== undefined ? String(state.batchNonce) : '0';
    }

    if (executorSigElement) {
      executorSigElement.value = state.executorSig || '';
    }
  }

  private copyParam(paramName: string): void {
    ...
    try {
      ...
      // 복사 성공 피드백
      const buttonId = `copy${paramName.charAt(0).toUpperCase() + paramName.slice(1)}Btn`;
      ...
      alert('[ERROR] 복사에 실패했습니다: ' + (err as Error).message);
    }
  ...
  private async submit(): Promise<void> {
    ...
    try {
      ...
      if (!state.userBatchSigs || state.userBatchSigs.length === 0) {
        alert('User Batch 서명이 없습니다. Step 3에서 서명해주세요.');
        return;
      }

      if (!state.executorSig) {
        alert('Executor 서명이 없습니다. Step 6에서 서명을 생성해주세요.');
        return;
      }

      // Batch Nonce 검증
      if (state.batchNonce === undefined || state.batchNonce === null) {
      ...
      if (!state.executorWallet) {
        alert('Executor Wallet이 설정되지 않았습니다.');
        return;
      }
      ...
      // userBatchSigs 기준으로 검증 (각 유저의 모든 레코드가 같은 votingId를 가져야 함)
      const byUser = new Map<string, bigint>();
      for (const sig of state.userBatchSigs) {
        // 현재 데모에서는 모든 레코드가 같은 votingId를 가진다고 가정
        // 첫 번째 레코드의 votingId를 해당 유저의 votingId로 설정
        if (state.records.length > 0) {
          const votingId = state.records[0]?.votingId;
          if (votingId !== undefined) {
            byUser.set(sig.user, votingId);
          }
        }
      }

      // 기존 결과/에러 숨기기
      UIHelper.hide('submitResult');
      UIHelper.hide('submitError');

      // UserVoteBatch[] 구조로 변환 (recordId 포함)
      let globalIndex = 0;
      const batches = state.userBatchSigs.map((sig) => {
        // 모든 레코드를 해당 유저의 것으로 처리 (데모 목적)
        const userRecords = state.records.map((r) => {
          const recordId = generateRecordId(globalIndex++);
          // 모든 uint256 필드를 BigInt로 변환
          return [
        ...
        return [
          ...
          [
            ...
            sig.user,
            BigInt(sig.nonce), // userNonce: uint256
            sig.signature,
          ],
        ];
      });

      const batchNonce = BigInt(state.batchNonce);
      const executorSig = state.executorSig;

      // Viem Client 가져오기
      this.setLoadingState(true, 'Viem 클라이언트 준비 중...');
      const publicClient = state.executorWallet.getPublicClient() as PublicClient;
      const walletClient = state.executorWallet.getWalletClient() as WalletClient;
      ...
      const hash = (await walletClient.writeContract({
        ...
        args: [batches as any, batchNonce, executorSig],
      } as any)) as `0x${string}`;

      // 트랜잭션 대기
      this.setLoadingState(true, `트랜잭션 확인 대기 중... (TX: ${hash.slice(0, 10)}...)`);
      const receipt = await publicClient.waitForTransactionReceipt({ hash });

      // 성공 UI 업데이트
      this.setLoadingState(false);
      UIHelper.setText('txHash', hash);
      const explorerUrl = `https://testnet.opbnbscan.com/tx/${hash}`;
      UIHelper.setAttribute('txLink', 'href', explorerUrl);
      UIHelper.show('submitResult');
      UIHelper.hide('submitError');
      ...
      if (this.onEventsReceived && receipt) {
        ...
        try {
          this.onEventsReceived(receipt, hash);
        } catch (eventError) {
      ...
      UIHelper.setText('errorMessage', errorMessage);
      UIHelper.show('submitError');
      UIHelper.hide('submitResult');
    }
  }

  private setLoadingState(isLoading: boolean, statusText: string = ''): void {
    ...
    if (isLoading) {
      ...
      UIHelper.setInnerHTML(
        'submitButtonText',
        '<i data-lucide="clock" class="w-4 h-4 inline animate-spin"></i> 처리 중...'
      );
      UIHelper.show('submitLoading');
      ...
      UIHelper.setInnerHTML(
        'submitButtonText',
        '<i data-lucide="rocket" class="w-4 h-4 inline"></i> 컨트랙트에 제출'
      );
      UIHelper.hide('submitLoading');
    }
  }
}
