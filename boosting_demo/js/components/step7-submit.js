  ...
  async submit() {
    try {
      ...
      if (!this.state.userSigs || this.state.userSigs.length === 0) {
        alert('User 서명이 없습니다. Step 3에서 서명해주세요.');
        return;
      }

      // 1:1 매핑 검증 (레코드 수 === 서명 수)
      if (this.state.records.length !== this.state.userSigs.length) {
        alert(`레코드와 서명 개수가 일치하지 않습니다!\n레코드: ${this.state.records.length}개\n서명: ${this.state.userSigs.length}개\n\n각 레코드마다 1개씩 서명이 필요합니다.`);
        return;
      }
