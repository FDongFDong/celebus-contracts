...
import { getBlockchainErrorMessage, isUserRejectionError, logError } from '@/lib/error-handler';

export function SubStep6Digest() {
  ...
  const setSubVotingExecutorSig = useAppStore((s) => s.setSubVotingExecutorSig);
  const setSubVotingDigest = useAppStore((s) => s.setSubVotingDigest);
  ...
  useEffect(() => {
    setBatchNonce(subVotingBatchNonce);
    if (subVotingBatchNonce !== null && subVotingBatchNonce !== undefined) {
      setStructHash(DigestService.calculateStructHash(subVotingBatchNonce));
    }
  }, [subVotingBatchNonce]);
  ...
  const handleBatchNonceChange = (value: string) => {
    ...
    setSubVotingBatchNonce(nextNonce);
  ...
  const handleGenerateSignature = async () => {
    try {
      ...
      setSubVotingExecutorSig(signature as Hash);
      ...
      logError('SubStep6Digest.handleGenerateSignature', error);
  ...
  return (
    <StepCard
      ...
      description="SubVoting 제출용 배치 다이제스트를 계산하고 Executor 서명을 생성합니다"
      badgeColor="default"
    >
      <div className="space-y-4">
        <div className="space-y-3">
          <div className="flex flex-col gap-1.5">
            <Label htmlFor="subDigestDomainSeparator">도메인 구분자 (STEP 4)</Label>
            <Input
              id="subDigestDomainSeparator"
              type="text"
          ...
          <div className="flex flex-col gap-1.5">
            <Label htmlFor="subDigestStructHash">구조체 해시 (STEP 5)</Label>
            <Input
              id="subDigestStructHash"
              type="text"
              value={structHash ?? ''}
              readOnly
          ...
          <div className="flex flex-col gap-1.5">
            <Label htmlFor="subDigestBatchNonce">배치 Nonce</Label>
            <Input
              id="subDigestBatchNonce"
              type="text"
        ...
        {finalDigest && (
          <div className="flex flex-col gap-1.5">
            <Label htmlFor="subFinalDigest">최종 다이제스트</Label>
            <Input
              id="subFinalDigest"
              type="text"
              value={finalDigest}
