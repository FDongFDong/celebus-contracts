...
const REASON_CODES: Record<number, { name: string; message: string }> = {
  ...
  4: { name: 'INVALID_BOOST_TYPE', message: '잘못된 boostingWith 타입' },
  5: { name: 'ARTIST_NOT_ALLOWED', message: '허용되지 않은 아티스트' },
  6: { name: 'STRING_TOO_LONG', message: '문자열 길이 초과' },
  7: { name: 'ZERO_AMT', message: '부스팅 수량이 0' },
  8: { name: 'DUPLICATE_HASH', message: '중복 레코드' },
};

export function BoostStep9Events() {
  ...
  const queryByTxHash = async () => {
    ...
    try {
      ...
      const parsedEvents: UserMissionResultEvent[] = parsedLogs.map((log) => {
        ...
        return {
          boostingId: args.boostingId?.toString() || '0',
  ...
  return (
    <StepCard
      stepNumber={9}
      title="이벤트 조회 및 알림"
      description="Boosting UserMissionResult 이벤트를 조회하고 실패 사유를 확인합니다"
    >
            ...
            <table className="w-full text-sm">
              <thead className="bg-muted/50">
                <tr>
                  ...
                  <th className="px-4 py-2 text-left font-medium">Boosting ID</th>
                ...
                {events.map((event, i) => {
                  ...
                  return (
                    <tr key={i} className="border-t hover:bg-muted/30">
                      ...
                      <td className="px-4 py-2 font-mono text-xs">{event.boostingId}</td>
